...
vector<string> LTDPViterbi_Oblivious::solve(const vector<string>& sequence)
{
    ...
    // till  convergence (fix up loop)
    bool converged = false;
    int log_t = int(log2(seq_size)) + 1;
    for (int c = min(int(log_t - 3, 2)); j < log_t && !converged; c++)
    {
        int size = pow(2, c);
        int q = seq_size / (2 * size);
        #pragma omp parallel num_threads(q-1) shared(state_list, obs_list, seq_size, state_size, viterbi, pred, conv)
        {
